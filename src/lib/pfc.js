...
class PFCAssistant extends Assistant {
  ...
  analyze(input) {
    return this.retryWithExponentialBackoff(() => {
      return new Promise((resolve, reject) => {
        limiter.schedule(() => openai.beta.threads.create())
          .then(thread => {
            return limiter.schedule(() => this.ask(input, thread.id))
              .then(result => {
                return limiter.schedule(() => openai.beta.threads.del(thread.id))
                  .then(() => resolve(result));
              });
          })
          .catch(error => {
  ...
  retryWithExponentialBackoff(operation, maxRetries = 5, baseDelay = 1000) {
    return new Promise((resolve, reject) => {
...
Provide insights on task complexity, attention allocation, and goal-directed behavior.`,
        ...
        .then(assistant => resolve(new DLPFCAssistant(assistant)))
        .catch(error => {
...
Provide insights on risk assessment, social context, and personal relevance.`,
        ...
        .then(assistant => resolve(new VMPFCAssistant(assistant)))
        .catch(error => {
...
Provide insights on reward magnitude, probability, and delay considerations.`,
        ...
        .then(assistant => resolve(new OFCAssistant(assistant)))
        .catch(error => {
...
Provide insights on perspective-taking, mentalizing, and social norms.`,
        ...
        .then(assistant => resolve(new MPFCAssistant(assistant)))
        .catch(error => {
...
Provide insights on performance monitoring, adaptive behavior, and motivation.`,
        ...
        .then(assistant => resolve(new ACCAssistant(assistant)))
        .catch(error => {
...
Provide insights on exploration vs exploitation, value function approximation, and policy improvement in the context of PFC functions.`,
        ...
        .then(assistant => resolve(new QLearningAssistant(assistant)))
        .catch(error => {
  ...
  analyze(input, otherAgentOutputs) {
    return this.retryWithExponentialBackoff(() => {
      return new Promise((resolve, reject) => {
        limiter.schedule(() => openai.beta.threads.create())
          .then(thread => {
            return limiter.schedule(() => this.ask(input, thread.id))
              .then(response => {
                ...
                return this.qlearning.act(state)
                  .then(actions => {
                    ...
                    if (typeof this.qlearning.learn === 'function') {
                      return this.qlearning.learn(state, actions, reward, nextState)
                        .then(() => combinedOutput);
                    } else {
                      console.warn('QLearning learn method not available');
...
function createAssistants() {
  return Promise.all([
    DLPFCAssistant.createInstance(),
    VMPFCAssistant.createInstance(),
    OFCAssistant.createInstance(),
    MPFCAssistant.createInstance(),
    ACCAssistant.createInstance(),
    QLearningAssistant.createInstance()
  ])
    .then(([dlpfc, vmpfc, ofc, mpfc, acc, qlearning]) => {
      return {
        DLPFC: dlpfc,
        VMPFC: vmpfc,
        OFC: ofc,
        ...
        QLearning: (input) => {
          return Promise.all([
            ...
            .then(otherAgentOutputs => {
              return qlearning.analyze(input, {
                DLPFC: otherAgentOutputs[0],
                VMPFC: otherAgentOutputs[1],
                OFC: otherAgentOutputs[2],
                MPFC: otherAgentOutputs[3],
                ACC: otherAgentOutputs[4],
              });
            })
            .catch(error => {
              console.error("Error in QLearning analysis:", error);
              throw error;
            });
        }
      };
